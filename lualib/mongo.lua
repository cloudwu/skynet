...
function mongo_cursor:sort(key_list)
	self.__sort = bson_encode(key_list)
	return self
end
...
function mongo_cursor:hasNext()
	if self.__ptr == nil then
		...
		if self.__data == nil then
			local query = nil
			if not self.__sort then
				query = self.__query
			else
				query = {['$query'] = self.__query, ['$orderby'] = self.__sort}
				query = bson_encode(query)
			end
			pack = driver.query(request_id, self.__flags, self.__collection.full_name, 
					self.__skip, -self.__limit, query, self.__selector)
		else
