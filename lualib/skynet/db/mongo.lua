...
function mongo_db:start_transaction(fun_transaction)
...
function mongo.enable_transaction(enable_transaction)
	if enable_transaction then
		function mongo_db:runCommand(cmd,cmd_v,...)
		...
		mongo_db.runCommand = old_runCommand
		mongo_db.send_command = old_send_command
	end
end

local run_test_transaction = false
function mongo_db:test_transaction_()
	if run_test_transaction then
		return
	end
	run_test_transaction = true
	mongo.enable_transaction(true)
	...
	print(self:start_transaction(function()
		print("-----------------------1")
		...
		print(db:insert({_id = bson.objectid(), key = 4, d = 4, by = "send"}))
