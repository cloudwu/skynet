...
skynet_socket_poll() {
	...
	case SOCKET_WARNING:
		...
		break;
	case SOCKET_EMPTY:
		forward_message(SKYNET_SOCKET_TYPE_EMPTY, false, &result);
		break;
	default:
...
void
skynet_socket_warnsize(struct skynet_context *ctx, int id, int warnsz){
	socket_server_warnsize(SOCKET_SERVER, id, warnsz);
}
